...
public class GitHubAPIHttpClient {
    ...
    protected JSONArray getNextIssues(Integer page, Instant since) throws InterruptedException {
        ...
        try {
            ...
            if (headers.keySet().contains("X-RateLimit-Limit")) {
                XRateLimit = Integer.valueOf(headers.getFirst("X-RateLimit-Limit"));
            } else {
                XRateLimit = 9999;
            }

            if (headers.keySet().contains("X-RateLimit-Remaining")) {
                XRateLimit = Integer.valueOf(headers.getFirst("X-RateLimit-Remaining"));
            } else {
                XRateLimit = 9999;
            }

            if (headers.keySet().contains("X-RateLimit-Reset")) {
                XRateReset = Integer.valueOf(headers.getFirst("X-RateLimit-Reset"));
            } else {
                XRateReset = Instant.MAX.getEpochSecond();
            }
